...
class ODHLV_MaxMind_DB_Reader {
	...
	private function parse_metadata() {
		...
		$decoder = new ODHLV_MaxMind_DB_Decoder( $raw_metadata, 0 );
	...
	public function lookup( $ip_address ) {
		...
		for ( $i = 0; $i < strlen( $bits ); $i++ ) {
			...
			if ( $next >= $node_count ) {
				...
				$decoder = new ODHLV_MaxMind_DB_Decoder( $raw_data, 0 );
...
class ODHLV_MaxMind_DB_Decoder {
